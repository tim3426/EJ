USE [ejdevo]
GO
/****** Object:  StoredProcedure [dbo].[USP_DATALIST_ADHOCQUERY_A0D1B673_F4A5_497B_8113_9DAEB025E8B2]    Script Date: 21-Aug-19 12:50:58 PM ******/
SET ANSI_NULLS ON
GO
...
select distinct C.CONSTITUENTID as [PROSPECTID]
	...
	,LASTMOVEOWNER.[Name] as [Last Move Owner]
	,NEXTMOVE.[Objective] as [Next Move]
	,NEXTMOVE.[EXPECTEDDATE] as [Next Move Date]
	,NEXTMOVEOWNER.[Name] as [Next Move Owner]
...
from EJ_CONSTITUENT_HISTORY as C
	...
	**/
	...
	left join EJ_CONSTITUENT_HISTORY as NEXTMOVEOWNER on NEXTMOVE.OWNERID = NEXTMOVEOWNER.CONSTITUENTID
	...
	left join 
			...
			,OWNERID
			,ACTUALDATE
			,V_QUERY_INTERACTIONALL.CONSTITUENTID
			,ROW_NUMBER() OVER (PARTITION BY V_QUERY_INTERACTIONALL.CONSTITUENTID ORDER BY ACTUALDATE DESC) as ROWNUMBER
			FROM V_QUERY_INTERACTIONALL
	...
	left join EJ_CONSTITUENT_HISTORY as LASTMOVEOWNER on LASTMOVE.OWNERID = LASTMOVEOWNER.CONSTITUENTID
